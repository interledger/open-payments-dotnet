...
### 8. First-class pagination: `IAsyncEnumerable` auto-paging — done (Phase 4)
List endpoints return raw `ListIncomingPaymentsResponse` with a cursor the caller must loop manually. ~~Add~~ Added `IAsyncEnumerable<IncomingPayment>`/`IAsyncEnumerable<OutgoingPayment>`-returning overloads (`ListIncomingPaymentsAllAsync`, `ListOutgoingPaymentsAllAsync` on `OpenPayments.Sdk/Clients/ResourceClientBase.cs`, `IResourceClientBase`, `AuthenticatedClient.cs`, `IAuthenticatedClient`) that follow `pageInfo.EndCursor` automatically while `HasNextPage` is true, alongside the existing page-at-a-time methods; they reject `Last`-set (backward-paging) queries up front and guard against a repeated cursor. This was the single biggest day-to-day ergonomics win for consumers. Covered by `OpenPayments.Sdk.Tests/Clients/ResourceClientBase_PagingTests.cs` and `AuthenticatedClient_PagingTests.cs`.

### 9. Make serialization consistent (and plan the System.Text.Json migration) — resolver unification done (Phase 4)
`AuthContractResolver` relaxed `Required.Always` and ignored nulls while `ResourceContractResolver` was an empty pass-through — so resource-server responses could throw on spec-vs-server drift that auth responses tolerated. Short term (done): unified into a single `OpenPaymentsContractResolver` (`OpenPayments.Sdk/Serialization/OpenPaymentsSerialization.cs`) that now backs all four HTTP clients (`AuthServerClient`, `ResourceServerClient`, `WalletAddressClient`, `UnauthenticatedClient`) — the old per-client resolver divergence is gone, and the same leniency applies uniformly to both response deserialization and request-body serialization (null-valued optional properties are omitted rather than emitted as `"prop": null`). Covered by `SerializationDrift_Tests.cs` (response deserialization) and `SerializationNullOmission_Tests.cs` (request-body null omission). Longer term (still open): move generation and hand-written code to System.Text.Json (source-generated contexts) — drops the Newtonsoft dependency and enables trimming/AOT support, which modern .NET consumers increasingly expect; see `docs/adr/0001-system-text-json-migration.md`.
